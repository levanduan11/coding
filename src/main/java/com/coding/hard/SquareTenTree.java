...
public class SquareTenTree {
    public static void main(String[] args) {
       ...
       String strL=new BigInt(scan.next()).subtract(BigInt.ONE).toString(); // subtract 1
       String strR=scan.next();
       scan.close();

       int[]intervalDigits = new int[log2(strR.length())+3];
        for (int k = 0; k < intervalDigits.length; k++) {
            intervalDigits[k]=digitsInterval(k);
        }
    ...
    static int digitsInterval(int k) {
        if (k == 0) return 1;
        else return (int) (Math.pow(2, k - 1) + 1);
    }
...
class BigInt {
    ...
    public BigInt(String str) {
        ...
        for (int i = 0; i < str.length(); i++) {
            digits[i] = Byte.parseByte(str.substring(i, i + 1));
        }
    }
    ...
    public BigInt subtract(BigInt other) {
        ...
        while (ptrB >= 0 && ptrR >= 0) {
            ...
            if (result[ptrR] < 0) {
                result[ptrR] += 10;
                int prtBorrow = ptrR - 1;
                while (result[prtBorrow] == 0) {
                    result[prtBorrow--] = 9;
                }
                result[prtBorrow]--;
            }
    ...
    public boolean equals(Object other) {
        if (!(other instanceof BigInt)) return false;
        byte[] digitsA = digits;
