...
public class DeterminingDNAHealth {
    ...
    void solve() {
        ...
        for (long e : es) {
            ...
            int pm = (e & 1) == 0 ? -1 : 1;
            while (p < n && p <= x - 1) {
                int d = Integer.numberOfLeadingZeros(p + 1);
                tries[d] = new TrieByLink();
                for (int k = p - (1 << d) + 1; k <= p; k++) {
                    tries[d].add(ss[k], h[k]);
                }
    ...
    public static class TrieByLink {
        public Node root = new Node((char) 0, 0);
        public int gen = 1;
        public static final char[] atoz = "abcdefghijklmnopqrstuvwzyz".toCharArray();
